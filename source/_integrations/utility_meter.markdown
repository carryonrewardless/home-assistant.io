...
The **Utility Meter** {% term integration %} provides functionality to track consumptions of various utilities (e.g., energy, gas, water, heating). Note that this does not integrate the input sensor. The input must be an energy (kWh or Wh) or quantity (litres, gallons, kg etc) type of sensor.
